if __name__ == "__main__":
    ...
    from modbus_servers_manager_config import SERVER_MANAGER_FILEPATH
    ...
    def add_server():
        ...
        MODBUS_SERVER_CONFIGS.append(new_server)
        with open(SERVER_MANAGER_FILEPATH, "w") as config_file:
            config_file.write(f"MODBUS_SERVER_CONFIGS = {str(MODBUS_SERVER_CONFIGS)}")
    ...
    def delete_selected_server():
        ...
        if selected_index:
            MODBUS_SERVER_CONFIGS.pop(selected_index[0])
            with open(SERVER_MANAGER_FILEPATH, "w") as config_file:
                config_file.write(f"MODBUS_SERVER_CONFIGS = {str(MODBUS_SERVER_CONFIGS)}")
            update_server_listbox()
    ...
    def update_server_listbox():
        ...
        for server in MODBUS_SERVER_CONFIGS:
            servers_listbox.insert(tk.END, f"{server['host']}:{server['port']}")

    # Create the main window
    root = tk.Tk()
    root.title("ModBus Server Configuration GUI")
